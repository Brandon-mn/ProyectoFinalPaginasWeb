          ...
          <div class="table-container active" id="tabla1">
            ...
            <table class="table">
              ...
              <tbody>
                ...
                $consulta = "SELECT id_tratamiento, descripcion, duracion, dosis, costo, id_enfermedad FROM tratamiento WHERE estatus = 1";
                ...
                while ($fila = mysqli_fetch_assoc($resultados)) {
                  ...
                  $consultaPacientes = "SELECT id_enfermedad, nombre FROM enfermedad";
                  $resultadoPacientes = $conexion->query($consultaPacientes);

                  echo '<td>
                          <select name="enfermedad" class="custom-select" disabled>';
                  while ($filaPaciente = mysqli_fetch_assoc($resultadoPacientes)) {
                    echo '<option value="' . $filaPaciente['id_enfermedad'] . '"';
                    if ($filaPaciente['id_enfermedad'] == $fila['id_enfermedad']) {
                      echo ' selected';
                    }
                  ...
                  echo '<td>
                          <form action="confirmareliminarttratamiento.php" method="POST">
                            <input type="hidden" name="id" value="' . $fila['id_tratamiento'] . '">
                            <button type="submit" class="btn btn-danger btn-eliminar">Eliminar</button>
                          </form>

                          <a href="editardiseñotratamiento.php?id=' . $fila['id_tratamiento'] . '" class="btn btn-success btn-editar">Editar</a>
                        </td>';
            ...
            <li class="nav-item">
              <a href="diseñoinsertartratamiento.php" class="nav-link">
                <span class="text-black fw-bold btn btn-primary">Agregar</span>
              </a>
              <a href="exportar_pdftratamiento.php" target="_blank" class="btn btn-primary">Exportar a PDF</a>
              <a href="exportar_xlstratamiento.php" target="_blank" class="btn btn-primary">Exportar a XLS</a>
              <a href="exportar_csvtratamiento.php" target="_blank" class="btn btn-primary">Exportar a CSV</a>
              <a href="exportar_xlmtratamiento.php" target="_blank" class="btn btn-primary">Exportar a XLM</a>
              <a href="exportar_jasontratamientro.php" target="_blank" class="btn btn-primary">Exportar a JSON</a>
              <a href="generar_informe.php" target="_blank" class="btn btn-primary">Generar Informe</a>
            </li>
          </div>
        </div>
      </div>
