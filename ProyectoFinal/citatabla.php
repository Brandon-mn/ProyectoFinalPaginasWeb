...
$consulta = "SELECT id_cita, fecha, hora, id_paciente, id_medico FROM cita WHERE estatus = 1";
...
while ($fila = mysqli_fetch_assoc($resultados)) {
  ...
  echo '<td>
          <select name="medico" class="custom-select" disabled>';
  while ($filaMedico = mysqli_fetch_assoc($resultadoMedicos)) {
    echo '<option value="' . $filaMedico['id_medico'] . '"';
    if ($filaMedico['id_medico'] == $fila['id_medico']) {
      echo ' selected';
    }
    echo '>' . $filaMedico['nombre'] . '</option>';
  }
  echo '</select>
        </td>';

  echo '<td>
          <form action="confirmareliminarcita.php" method="POST">
            <input type="hidden" name="id" value="' . $fila['id_cita'] . '">
            <button type="submit" class="btn btn-danger btn-eliminar">Eliminar</button>
          </form>

          <a href="editardiseñocita.php?id=' . $fila['id_cita'] . '" class="btn btn-success btn-editar">Editar</a>
        </td>';
...
?>
            ...
            <li class="nav-item">
              <a href="diseñoinsertarcita.php" class="nav-link">
                <span class="text-black fw-bold btn btn-primary">Agregar</span>
              </a>
              <a href="exportar_pdfcita.php" target="_blank" class="btn btn-primary">Exportar a PDF</a>
<a href="exportar_xlscita.php" target="_blank" class="btn btn-primary">Exportar a XLS</a>
<a href="exportar_csvcita.php" target="_blank" class="btn btn-primary">Exportar a CSV</a>
<a href="exportar_xlmcita.php" target="_blank" class="btn btn-primary">Exportar a XLM</a>
<a href="exportar_jasoncita.php" target="_blank" class="btn btn-primary">Exportar a JSON</a>
<a href="generar_pdf_cita_proxima.php" target="_blank" class="btn btn-primary">Generar PDF de Cita Próxima</a>
